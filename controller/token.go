...
func validateToken(c *gin.Context, token model.Token) error {
	...
	if token.Subnet != nil && *token.Subnet != "" {
		...
		if err != nil {
			return fmt.Errorf("invalid subnet: %s", err.Error())
		}
	}
	return nil
}
...
func UpdateToken(c *gin.Context) {
	...
	if token.Status == model.TokenStatusEnabled {
		if cleanToken.Status == model.TokenStatusExpired && cleanToken.ExpiredTime <= helper.GetTimestamp() && cleanToken.ExpiredTime != -1 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "token has expired and cannot be enabled, please first modify the token expiration time, or set it to never expire",
			})
			return
		}
		if cleanToken.Status == model.TokenStatusExhausted && cleanToken.RemainQuota <= 0 && !cleanToken.UnlimitedQuota {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "token available quota exhausted and cannot be enabled, please first modify token remaining quota or set to unlimited",
			})
			return
		}
	}
