...
func (user *User) Insert(ctx context.Context, inviterId int) error {
	...
	if config.QuotaForNewUser > 0 {
		RecordLog(ctx, user.Id, LogTypeSystem, fmt.Sprintf("new user registration bonus %s", common.LogQuota(config.QuotaForNewUser)))
	}
	if inviterId != 0 {
		if config.QuotaForInvitee > 0 {
			_ = IncreaseUserQuota(user.Id, config.QuotaForInvitee)
			RecordLog(ctx, user.Id, LogTypeSystem, fmt.Sprintf("invitation code bonus %s", common.LogQuota(config.QuotaForInvitee)))
		}
		if config.QuotaForInviter > 0 {
			_ = IncreaseUserQuota(inviterId, config.QuotaForInviter)
			RecordLog(ctx, inviterId, LogTypeSystem, fmt.Sprintf("inviting user bonus %s", common.LogQuota(config.QuotaForInviter)))
		}
	}
...
func (user *User) ValidateAndFill() (err error) {
	...
	if err != nil {
		...
		if err != nil {
			return errors.New("username or password is incorrect, or user has been banned")
		}
	}
