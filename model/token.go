...
func ValidateUserToken(key string) (token *Token, err error) {
	...
	if token.Status == TokenStatusExhausted {
		return nil, fmt.Errorf("token %s (#%d) quota exhausted", token.Name, token.Id)
	} else if token.Status == TokenStatusExpired {
		return nil, errors.New("this token has expired")
	}
	if token.Status != TokenStatusEnabled {
		return nil, errors.New("this token status is unavailable")
	}
	...
	if !token.UnlimitedQuota && token.RemainQuota <= 0 {
		...
		return nil, errors.New("this token quota exhausted")
	}
	return token, nil
}
		...
		go func() {
			...
			if email != "" {
				...
				content := message.EmailTemplate(
					...
					fmt.Sprintf(`
						<p>Hello!</p>
						<p>%s, current remaining quota is <strong>%d</strong>.</p>
						<p>To ensure uninterrupted service, please top up in time.</p>
