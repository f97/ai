...
func Redeem(ctx context.Context, key string, userId int) (quota int64, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if redemption.Status != RedemptionCodeStatusEnabled {
			return errors.New("this redemption code has been used")
		}
	...
	RecordLog(ctx, userId, LogTypeTopup, fmt.Sprintf("topped up via redemption code %s", common.LogQuota(redemption.Quota)))
